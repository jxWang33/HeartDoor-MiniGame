...
#include "UnityCG.cginc"
		...
		half4 frag(v2f_img i) : SV_Target
		{
			...
			//最外层白圈
			if (length(p - _Drop.xy) < (_Drop.z / _Params1.z)
				&& length(p - _Drop.xy) > (_Drop.z / _Params1.z) - 0.005f) {
				return fixed4(1, 1, 1, 1);
			}

			float w = wave(p, _Drop.xy, _Drop.z);//振幅，用振幅来对当前点做UV上面的偏移，即可产生涟漪效果
			float2 dw = float2(wave(p+dx, _Drop.xy, _Drop.z) - w, wave(p+dy, _Drop.xy, _Drop.z) - w);//xy上振幅
